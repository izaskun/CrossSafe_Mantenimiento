...
public class RegistrationReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		Log.w("IKER", "Registration Receiver called");
		if ("com.google.android.c2dm.intent.REGISTRATION".equals(action)) {
			Log.w("IKER", "Received registration ID");
			final String registrationId = intent
					.getStringExtra("registration_id");
			String error = intent.getStringExtra("error");

			Log.d("IKER", "dmControl: registrationId = " + registrationId
					+ ", error = " + error);
	...
	private void saveRegistrationId(Context context, String registrationId) {
		...
		edit.putString(CrossSafe_Mantenimiento.AUTH, registrationId);
		edit.commit();
	}
	...
	public void sendRegistrationIdToServer(String deviceId,
		...
		Log.d("IKER", "Sending registration ID to my application server");
